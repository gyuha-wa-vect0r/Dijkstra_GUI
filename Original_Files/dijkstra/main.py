...
def on_node_hover(graph):
    ...
    for node in graph.nodes:
        node.hovered = False
        if pygame.mouse.get_pos()[0] + node.radius >= node.pos[0] >= pygame.mouse.get_pos()[0] - node.radius and \
                pygame.mouse.get_pos()[1] + node.radius >= node.pos[1] >= pygame.mouse.get_pos()[1] - node.radius:
            node.hovered = True
...
def main():
    ...
    screen = pygame.display.set_caption('Dijkstra 2024')
